...
const STACK_BB = Number(process.env.BENCH_STACK_BB || 15);
...
return (async()=>{
  ...
  const cfrUsage={handled:0,fallback:0,nodes:{},actions:{}};
  ...
  const originalTryCfr=tryExecuteHuMidstackCfr;
  tryExecuteHuMidstackCfr=function(index,notation,toCall){
    const p=players[index];
    ...
    if(p&&p.benchmarkPolicy==="new"){
      if(result&&result.handled){
        cfrUsage.handled++;
        const node=result.nodeId||"unknown";
        const action=result.action||"unknown";
        cfrUsage.nodes[node]=(cfrUsage.nodes[node]||0)+1;
        cfrUsage.actions[action]=(cfrUsage.actions[action]||0)+1;
      }else{
        cfrUsage.fallback++;
      }
    }
  ...
  async function runHand(swapped,pairSeed){
    ...
    for(const p of players){
      ...
      p.huMidstackCfrMix=p.benchmarkPolicy==="new"?MIX_VALUE:0;
    }
...
(async()=>{
  ...
  console.log("HU_MIDSTACK_AB_RESULT="+JSON.stringify(result));
})().catch(err=>{
